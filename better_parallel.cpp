...
int main() {
...

    {
        ...
        for(int t = 1; t <= NUM_THREADS_INCREMENTS; ++t) {
            ...
            #pragma omp parallel for //first touch may gain something, but all data must be read in the algorithm, TODO think about this
            ...
            for(int i = 0; i<NUM_MEASUREMENTS; i++) {
                ...
                {
                    Boid local_boids_aos[NUM_BOIDS];
                    for(int j = 0; j<NUM_BOIDS; j++)//no need to wait for other threads, as we wait for all changes at the end of the iteration
                        local_boids_aos[j] = boids_aos[j];
                    ...
                    for (int j = 0; j < NUM_BOIDS; ++j) {//get the parameters
                        ...
                        for (int k = 0; k < NUM_BOIDS; ++k) {
                            ...
                            float dx = local_boids_aos[j].x - local_boids_aos[k].x;
                            float dy = local_boids_aos[j].y - local_boids_aos[k].y;
                            ...
                            if (std::abs(dx) < VISUAL_RANGE && std::abs(dy) < VISUAL_RANGE) {
                                ...
                                if (squared_dist < PROTECTED_RANGE * PROTECTED_RANGE) {// check if the boid is too close
                                    ...
                                    parameters_aos[j].avg_x += local_boids_aos[k].x;
                                    parameters_aos[j].avg_y += local_boids_aos[k].y;
                                    parameters_aos[j].avg_vx += local_boids_aos[k].vx;
                                    parameters_aos[j].avg_vy += local_boids_aos[k].vy;
                    ...
                    for(int j = 0; j < NUM_BOIDS; ++j) {//update the boids
                        if (parameters_aos[j].neighboring_count > 0) {
                            ...
                            float new_vx = local_boids_aos[j].vx;
                            float new_vy = local_boids_aos[j].vy;
                            float new_x = local_boids_aos[j].x;
                            float new_y = local_boids_aos[j].y;
                            new_vx += (parameters_aos[j].avg_x - local_boids_aos[j].x) * CENTERING_FACTOR + (parameters_aos[j].avg_vx - local_boids_aos[j].vx) * MATCHING_FACTOR;
                            new_vy += (parameters_aos[j].avg_y - local_boids_aos[j].y) * CENTERING_FACTOR + (parameters_aos[j].avg_vy - local_boids_aos[j].vy) * MATCHING_FACTOR;
                            ...
                            local_boids_aos[j].update(new_vx, new_vy, new_x, new_y);
                        }
                    }
                    #pragma omp for
                    for(int j = 0; j < NUM_BOIDS; j++) {
                        #pragma omp atomic
                        boids_aos[j] = local_boids_aos[j]; //FIXME atomic issues
                    }//implicit barrier is needed to ensure all boids are updated
                }
...

    {
        ...
        for(int t = 1; t <= NUM_THREADS_INCREMENTS; ++t) {
            ...
            for(int i = 0; i<NUM_MEASUREMENTS; i++) {
                ...
                {
                    PadBoid local_pad_boids_aos[NUM_BOIDS];
                    for(int j = 0; j < NUM_MEASUREMENTS; j++)
                        local_pad_boids_aos[j] = pad_boids_aos[j];
                    #pragma omp for schedule(static) nowait
                    for (int j = 0; j < NUM_BOIDS; ++j) {//get the parameters
                        ...
                        for (int k = 0; k < NUM_BOIDS; ++k) {
                            ...
                            float dx = local_pad_boids_aos[j].x - local_pad_boids_aos[k].x;
                            float dy = local_pad_boids_aos[j].y - local_pad_boids_aos[k].y;
                            ...
                            if (std::abs(dx) < VISUAL_RANGE && std::abs(dy) < VISUAL_RANGE) {
                                ...
                                if (squared_dist < PROTECTED_RANGE * PROTECTED_RANGE) {// check if the boid is too close
                                    ...
                                    pad_parameters_aos[j].avg_x += local_pad_boids_aos[k].x;
                                    pad_parameters_aos[j].avg_y += local_pad_boids_aos[k].y;
                                    pad_parameters_aos[j].avg_vx += local_pad_boids_aos[k].vx;
                                    pad_parameters_aos[j].avg_vy += local_pad_boids_aos[k].vy;
                    ...
                    for(int j = 0; j < NUM_BOIDS; ++j) {//update the boids
                        if (pad_parameters_aos[j].neighboring_count > 0) {
                            ...
                            float new_vx = local_pad_boids_aos[j].vx;
                            float new_vy = local_pad_boids_aos[j].vy;
                            float new_x = local_pad_boids_aos[j].x;
                            float new_y = local_pad_boids_aos[j].y;
                            new_vx += (pad_parameters_aos[j].avg_x - local_pad_boids_aos[j].x) * CENTERING_FACTOR + (pad_parameters_aos[j].avg_vx - local_pad_boids_aos[j].vx) * MATCHING_FACTOR;
                            new_vy += (pad_parameters_aos[j].avg_y - local_pad_boids_aos[j].y) * CENTERING_FACTOR + (pad_parameters_aos[j].avg_vy - local_pad_boids_aos[j].vy) * MATCHING_FACTOR;
                            ...
                            local_pad_boids_aos[j].update(new_vx, new_vy, new_x, new_y);
                        }
                    }
                    #pragma omp for
                    for(int j = 0; j < NUM_BOIDS; j++) {
                        #pragma omp atomic
                        pad_boids_aos[j] = local_pad_boids_aos[j];
                    }
                }
...

    {
        ...
        for(int t = 1; t <= NUM_THREADS_INCREMENTS; ++t) {
            ...
            for(int i = 0; i<NUM_MEASUREMENTS; i++) {
                ...
                {
                    BoidsList local_boids_soa;
                    local_boids_soa = BoidsList(boids_soa);
                    #pragma omp for schedule(static) nowait
                    for (int j = 0; j < NUM_BOIDS; ++j) {//get the parameters
                        ...
                        for (int k = 0; k < NUM_BOIDS; ++k) {
                            ...
                            float dx = local_boids_soa.x[j] - local_boids_soa.x[k];
                            float dy = local_boids_soa.y[j] - local_boids_soa.y[k];
                            ...
                            if (std::abs(dx) < VISUAL_RANGE && std::abs(dy) < VISUAL_RANGE) {
                                ...
                                if (squared_dist < PROTECTED_RANGE * PROTECTED_RANGE) {// check if the boid is too close
                                    ...
                                    parameters_soa.avg_x[j] += local_boids_soa.x[k];
                                    parameters_soa.avg_y[j] += local_boids_soa.y[k];
                                    parameters_soa.avg_vx[j] += local_boids_soa.vx[k];
                                    parameters_soa.avg_vy[j] += local_boids_soa.vy[k];
                    ...
                    for(int j = 0; j < NUM_BOIDS; ++j) {//update the boids
                        if (parameters_soa.neighboring_count[j] > 0) {
                            ...
                            float new_vx = local_boids_soa.vx[j];
                            float new_vy = local_boids_soa.vy[j];
                            float new_x = local_boids_soa.x[j];
                            float new_y = local_boids_soa.y[j];
                            new_vx += (parameters_soa.avg_x[j] - local_boids_soa.x[j]) * CENTERING_FACTOR + (parameters_soa.avg_vx[j] - local_boids_soa.vx[j]) * MATCHING_FACTOR;
                            new_vy += (parameters_soa.avg_y[j] - local_boids_soa.y[j]) * CENTERING_FACTOR + (parameters_soa.avg_vy[j] - local_boids_soa.vy[j]) * MATCHING_FACTOR;
                            ...
                            local_boids_soa.update(j, new_vx, new_vy, new_x, new_y);
                        }
                    }
                    #pragma omp for
                    for(int j = 0; j < NUM_BOIDS; j++) { //many atomics, maybe critical update could be an option
                        #pragma omp atomic
                        boids_soa.x[j] = local_boids_soa.x[j];
                        #pragma omp atomic
                        boids_soa.y[j] = local_boids_soa.y[j];
                        #pragma omp atomic
                        boids_soa.vx[j] = local_boids_soa.vx[j];
                        #pragma omp atomic
                        boids_soa.vy[j] = local_boids_soa.vy[j];
                    }
                }
...

    {
        ...
        for(int t = 1; t <= NUM_THREADS_INCREMENTS; ++t) {
            ...
            for(int i = 0; i<NUM_MEASUREMENTS; i++) {
                ...
                {
                    PadBoidsList local_pad_boids_soa;
                    local_pad_boids_soa = pad_boids_soa;
                    ...
                    for (int j = 0; j < NUM_BOIDS; ++j) {//get the parameters
                        ...
                        for (int k = 0; k < NUM_BOIDS; ++k) {
                            ...
                            float dx = local_pad_boids_soa.x[j] - local_pad_boids_soa.x[k];
                            float dy = local_pad_boids_soa.y[j] - local_pad_boids_soa.y[k];
                            ...
                            if (std::abs(dx) < VISUAL_RANGE && std::abs(dy) < VISUAL_RANGE) {
                                ...
                                if (squared_dist < PROTECTED_RANGE * PROTECTED_RANGE) {// check if the boid is too close
                                    ...
                                    pad_parameters_soa.avg_x[j] += local_pad_boids_soa.x[k];
                                    pad_parameters_soa.avg_y[j] += local_pad_boids_soa.y[k];
                                    pad_parameters_soa.avg_vx[j] += local_pad_boids_soa.vx[k];
                                    pad_parameters_soa.avg_vy[j] += local_pad_boids_soa.vy[k];
                    ...
                    for(int j = 0; j < NUM_BOIDS; ++j) {//update the boids
                        if (pad_parameters_soa.neighboring_count[j] > 0) {
                            ...
                            float new_vx = local_pad_boids_soa.vx[j];
                            float new_vy = local_pad_boids_soa.vy[j];
                            float new_x = local_pad_boids_soa.x[j];
                            float new_y = local_pad_boids_soa.y[j];
                            new_vx += (pad_parameters_soa.avg_x[j] - local_pad_boids_soa.x[j]) * CENTERING_FACTOR + (pad_parameters_soa.avg_vx[j] - local_pad_boids_soa.vx[j]) * MATCHING_FACTOR;
                            new_vy += (pad_parameters_soa.avg_y[j] - local_pad_boids_soa.y[j]) * CENTERING_FACTOR + (pad_parameters_soa.avg_vy[j] - local_pad_boids_soa.vy[j]) * MATCHING_FACTOR;
                            ...
                            local_pad_boids_soa.update(j, new_vx, new_vy, new_x, new_y);
                        }
                    }
                    #pragma omp for
                    for(int j = 0; j < NUM_BOIDS; j++) { //many atomics, maybe critical update could be an option
                        #pragma omp atomic
                        pad_boids_soa.x[j] = local_pad_boids_soa.x[j];
                        #pragma omp atomic
                        pad_boids_soa.y[j] = local_pad_boids_soa.y[j];
                        #pragma omp atomic
                        pad_boids_soa.vx[j] = local_pad_boids_soa.vx[j];
                        #pragma omp atomic
                        pad_boids_soa.vy[j] = local_pad_boids_soa.vy[j];
                    }
                }
